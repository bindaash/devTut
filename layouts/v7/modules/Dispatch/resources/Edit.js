Vtiger_Edit_Js("Dispatch_Edit_Js", { 
	...
	updateRowNumberForRow : function(lineItemRow, expectedSequenceNumber, currentSequenceNumber){
		...
		var idFields = new Array('hdnProductId','productName','disp_quantity','searchIcon', 'lineItemType',  'totalItemCount');
	...
	registerEventForCurrentDate : function(container){
		...
		jQuery("input[name='dispatch_date']").val(currentDate).prop('readonly', true);
	},	
	/**
	...
	registerProductSelector : function() { 
        var self = this;
	...
	mapResultsToFields: function(parentRow,responseData){ 
	 ...
	 /**
	 * Function which will handle the actions that need to be preformed once the quantity is changed like below
	...
	getQuantityValue : function(lineItemRow){
		return parseFloat(lineItemRow.find('.quantity').val());
	},
	...
	registerQuantityChangeEvent : function() { 
        var self = this;
		this.lineItemsHolder.on('focusout','.quantity',function(e){
	...
	registerLineItemEvents : function() {
        ...
        if (!record) {
            ...
            jQuery('.quantity',container).trigger('focusout');
        }
    },
