Vtiger_Edit_Js("BatchMovement_Edit_Js", { 
	...
	updateRowNumberForRow : function(lineItemRow, expectedSequenceNumber, currentSequenceNumber){
		...
		var idFields = new Array('batchcode','hdnBatchId','lineItemType','quantity','searchIcon','batcmov_select_frm','batcmov_select_to','totalBatchCount');
		...
		jQuery('#totalBatchCount').val(total_tr);
	...
	registerAddBatch : function() { 
        ...
        var addLineItemEventHandler = function(e, data){
			if(jQuery('#lineItemTab').find('.lineItemRow').length>4){
				...
				newLineItem.find('input.batchcode').addClass('autoComplete');
	...
	checkLineItemRow : function(){ 
		...
		if(numRows > 1) { 
			var tr = jQuery('#totalBatchCount').val(numRows);
			this.showLineItemsDeleteIcon();
		}else{ 
			var tr1 = jQuery('#totalBatchCount').val(numRows);
	...
	registerEventForCurrentDate : function(container){
		...
		jQuery("input[name='batch_mov_date']").val(currentDate).prop('readonly', true);
	},	
	/**
	...
	searchModuleNames : function(params) {
		...
		if(params.search_module == 'Batch') {
            params.module = 'BatchMovement';
        }
	...
	mapResultsToFields: function(parentRow,responseData){
		var lineItemNameElment = jQuery('input.batchcode',parentRow);
	...
	registerClearLineItemSelection : function() { 
         ...
         this.lineItemsHolder.on('click','.clearLineItem', function(e){
            ...
            parentElem.find('input.batchcode').removeAttr('disabled').val('');
            e.preventDefault();
         });
	},
	...
	registerDeleteLineItemEvent : function(){
		...
		self.checkLineItemRow();
		var total_tr = jQuery('#lineItemTab tr').length;
		jQuery('#totalBatchCount').val(total_tr-1);
		});
	},
