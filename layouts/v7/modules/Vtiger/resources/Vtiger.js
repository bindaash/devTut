...
Vtiger.Class('Vtiger_Index_Js', {
	...
	registerEvents: function() {
		...
		this.registerOpenReportList();
		this.summary_of_batch_rep();
...
//			});
	...
	//Added by Biraj Sharma on date 02052019 for Custom Reports function
	registerOpenReportList : function() {	
		jQuery('#openReportList').on('click',function(e){
			jQuery("#report_new_popup").dialog({ 
				resizable: false,
				height:300,
				width:900,
				model:true
			...
			jQuery('#select_type').change(function(){
				var selectedType = jQuery(this).children("option:selected").val();
					if(selectedType == 'salesOrder_id'){
						jQuery.ajax({
							type:'POST',
							url:'report_ajax.php?case=get_sales_order_no',
							success:function(data){
							jQuery('#item_id').html(data);
								
							}
						})
					}else if(selectedType == 'product_id'){
						jQuery.ajax({
							type:'POST',
							url:'report_ajax.php?case=get_products_id',
							success:function(data){
							jQuery('#item_id').html(data);
						...
						alert(" You havn't selected any type ");
					}
			});
			//console.log('test');
		});
	...
	summary_of_batch_rep : function(){
		jQuery('#batch_reports_ware').on('click', function(e){
			
			if(jQuery('#war_dat').val()==''){
				alert('Please select Any Date first !');
			}else{
				var myValKey = { date : jQuery('#war_dat').val() }
				$.ajax({
				      type: 'POST',
				      url: "dompdf/examples/report_summary_of_warehouse.php",
				      data: myValKey,
				      dataType: "text",
				      success: function(date) {
				      	console.log(date);
				      	if(date == 'Success'){
				      		window.open("dompdf/examples/report_summary_of_warehouse.php", '_blank');
				      	}else{
				      		alert('You have some error. Please try again');
				      	}
				      }
				});
			}	
		...
		jQuery('#batch_reports_core').on('click', function(e){
			
			if(jQuery('#from_dat').val() =='' || jQuery('#to_dat').val() ==''){
				alert('Please select From and To first !');
			}else{
				var myKeyVals = { from : jQuery('#from_dat').val(), to : jQuery('#to_dat').val() }
				$.ajax({
				      type: 'POST',
				      url: "dompdf/examples/report_summary_of_core.php",
				      data: myKeyVals,
				      dataType: "text",
				      success: function(resultData) {
				      	console.log(resultData);
				      	if(resultData == 'Success'){
				      		window.open("dompdf/examples/report_summary_of_core.php",    '_blank');
				      	}else{
				      		alert('You have some error. Please try again');
				      	}
				      }
		...
		jQuery('#batch_reports').on('click', function(e){
			//console.log(jQuery('#item_id').val());
			var sel_type=jQuery('#select_type').val();
			var itemId=jQuery('#item_id').val();
			//console.log(prd_id);
			//alert(sel_type+'//'+itemId);
			if(sel_type =='' && itemId ==''){
			alert('Please select Sales Order Number Or Product Name first !');
			}else{
			window.open("dompdf/examples/report_summary_of_batch.php?node_type="+sel_type+"&node_id="+itemId,    '_blank');
			}	
			//console.log('Hello World');
		});
	},
