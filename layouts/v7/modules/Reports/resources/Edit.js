...
Vtiger_Edit_Js("Reports_Edit_Js",{
	...
	/*openReportList : function() {	console.log('Hello World');
		jQuery.ajax({
		   type:'POST',
		   url:'report_ajax.php?case=get_products_id',
			success:function(data){
			//jQuery('#vendor_id3').html(data);                                     
				jQuery.ajax({
				   type:'POST',
				   url:'report_ajax.php?case=get_branch',
				   success:function(data){
					   jQuery('#branch1').html(data);
					}
				   })
				jQuery("#report_popup").dialog({ 
					resizable: false,
					height:600,
					width:1000,
					model:true
