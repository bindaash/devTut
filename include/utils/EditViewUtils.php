...
{ 
	...
	elseif($module == 'Rejection'){
			$query='SELECT vtiger_products.productid, 
					...
					vtiger_products.product_no AS productcode, 
					vtiger_crmentity.deleted, "Products" AS entitytype,
					vtiger_movementrel.*,
					vtiger_rej_reason.rej_reason
					FROM vtiger_movementrel
					LEFT JOIN vtiger_crmentity ON vtiger_crmentity.crmid=vtiger_movementrel.itemid
					LEFT JOIN vtiger_products ON vtiger_products.productid=vtiger_movementrel.itemid
					LEFT JOIN vtiger_rej_reason ON vtiger_rej_reason.product_id=vtiger_movementrel.itemid
					WHERE id=? 
                    AND vtiger_movementrel.id = vtiger_rej_reason.rejectionid
                    AND vtiger_movementrel.itemid = vtiger_rej_reason.product_id
                    AND vtiger_movementrel.lineitem_id = vtiger_rej_reason.lineitem_id
		...
		$query ='SELECT 
					...
					vtiger_crmentity.deleted, "Products" AS entitytype,
					disp.*
				FROM vtiger_dispatch_item AS disp
					LEFT JOIN vtiger_crmentity ON vtiger_crmentity.crmid=disp.id
					LEFT JOIN vtiger_products ON vtiger_products.productid=disp.productid
				WHERE id=?
				ORDER BY sequence_no'; 
		$params = array($focus->id);		
	}
	...
	for($i=1;$i<=$num_rows;$i++)
	{
		...
		if($rej_reason != ''){
		$product_Detail[$i]['rej_reason'.$i]=$rej_reason;
		}
		//Rejection Dropdown not blanks
		if(!empty($disp_quantity)){
		$product_Detail[$i]['disp_quantity'.$i]=$disp_quantity;
		} else {
		$product_Detail[$i]['disp_quantity'.$i]=0;	
		}
	...
	//echo"<pre>";print_r($product_Detail); exit('Main');
	return $product_Detail;
}

function getAssociatedBatch($module, $focus, $seid = '', $refModuleName = false)
{	
	global $log;
	$log->debug("Entering getAssociatedBatch(".$module.",".get_class($focus).",".$seid."='') method ...");
	global $adb;
	$output = '';
	global $theme,$current_user;
	$batch_Detail = Array();
	if($module == 'BatchMovement'){ 
		$query='SELECT vtiger_batch.batchid, 
						vtiger_batch.batch_name, 
						vtiger_batch.batch_id AS batch_code, 
						vtiger_batch.batch_qty,
						vtiger_crmentity.deleted, "Batch" AS entitytype,
						vtiger_movementrel.*
					FROM vtiger_movementrel
						LEFT JOIN vtiger_crmentity ON vtiger_crmentity.crmid=vtiger_movementrel.itemid
	...
	$result = $adb->pquery($query, $params);
	$num_rows=$adb->num_rows($result); 
	//echo"<pre>";print_r($num_rows);exit('Hello');
	for($i=1;$i<=$num_rows;$i++)
	{
		//For Batch movement line item as batch details 
		$deleted = $adb->query_result($result,$i-1,'deleted');
		$hdnBatchId = $adb->query_result($result,$i-1,'batchid');
		$hdnBatchcode = $adb->query_result($result,$i-1,'batch_code');
		$batch_name=$adb->query_result($result,$i-1,'batch_name');
		//$qtyinstock=$adb->query_result($result,$i-1,'qtyinstock');
		$qty=$adb->query_result($result,$i-1,'quantity');
		$entitytype=$adb->query_result($result,$i-1,'entitytype');
		$batcmov_select_frm = $adb->query_result($result,$i-1,'select_frm');
		$batcmov_select_to = $adb->query_result($result,$i-1,'select_to');
		
		if(($deleted) || (!isset($deleted))){
			$batch_Detail[$i]['batchDeleted'.$i] = true;
		}elseif(!$deleted){
			$batch_Detail[$i]['batchDeleted'.$i] = false;
		}
		if (!empty($entitytype)) {
			$batch_Detail[$i]['entityType'.$i]=$entitytype;
		}
		if($qty ==''){
			$qty = 1;
		}else{
		$batch_Detail[$i]['quantity'.$i]=decimalFormat($qty);
		}
		
		//Delete link in First column
		if($i != 1)
		{
			$batch_Detail[$i]['delRow'.$i]="Del";
		}
		if($batcmov_select_frm || $batcmov_select_to != ''){
		$batch_Detail[$i]['batcmov_select_frm'.$i]=$batcmov_select_frm;
		$batch_Detail[$i]['batcmov_select_to'.$i]=$batcmov_select_to;
		}
		$batch_Detail[$i]['hdnBatchId'.$i] = $hdnBatchId;
...
function getAsBatchProductsSalesOrder($module, $focus, $seid = '', $refModuleName = false){
	global $log;
	$log->debug("Entering getAsBatchProductsSalesOrder(".$module.",".get_class($focus).",".$seid."='') method ...");
	global $adb;
	$output = '';
	global $theme,$current_user;
	$batch_Detail = Array();
	if($module == 'Batch'){
			$query='SELECT vtiger_crmentity.deleted, 
					vtiger_salesorder.subject, 
					vtiger_salesorder.salesorder_no, 
					vtiger_products.productname, 
					vtiger_products.product_no, 
					vtiger_batch_item.* 
					FROM vtiger_batch_item 
					LEFT JOIN vtiger_crmentity ON vtiger_crmentity.crmid=vtiger_batch_item.id 
					LEFT JOIN vtiger_salesorder ON vtiger_salesorder.salesorderid=vtiger_batch_item.salesorderid 
					LEFT JOIN vtiger_products ON vtiger_products.productid=vtiger_batch_item.productid 
					WHERE id=? ORDER BY sequence_no'; 
			$params = array($focus->id);
	}
	...
	for($i=1;$i<=$num_rows;$i++)
	{
		//For Batch movement line item as batch details 
		$deleted = $adb->query_result($result,$i-1,'deleted');
		$hdnSalesOrderId = $adb->query_result($result,$i-1,'salesorderid');
		$subject = $adb->query_result($result,$i-1,'subject');
		$salesorder_no = $adb->query_result($result,$i-1,'salesorder_no');
		$hdnProductId = $adb->query_result($result,$i-1,'productid');
		$productName = $adb->query_result($result,$i-1,'productname');
		$product_no = $adb->query_result($result,$i-1,'product_no');
		$sequence_no = $adb->query_result($result,$i-1,'sequence_no');
		$batchItem_id = $adb->query_result($result,$i-1,'batchItem_id');
		...
		if($hdnProductId != ''){
		$batch_Detail[$i]['hdnProductId'.$i]=$hdnProductId;
		}
		if($hdnSalesOrderId != ''){
		$batch_Detail[$i]['hdnSalesOrderId'.$i]=$hdnSalesOrderId;
		}
		$batch_Detail[$i]['salesorder_no'.$i] = $salesorder_no;
		$batch_Detail[$i]['subject'.$i] = from_html($subject);
		$batch_Detail[$i]['product_no'.$i] = $product_no;
		$batch_Detail[$i]['productName'.$i] = from_html($productName);
	}
	//echo"<pre>";print_r($batch_Detail); exit('Main');
	return $batch_Detail;
