...
function saveMovementProdBatchDetails(&$focus, $module, $update_prod_stock='false', $updateDemand='')
{	//echo $focus.'##'.$module.'##'.$update_prod_stock.'##'.$updateDemand."<br>"; die('Hy');
	//echo"<pre>";print_r($updateDemand)."<br>"; die('Hy');
	global $log, $adb;
	$id=$focus->id;
...
function saveMovementProdRejectionDetails(&$focus, $module)
{	global $log, $adb;
	$id=$focus->id;
	$log->debug("Entering into function saveMovementProdRejectionDetails($module).");
	$ext_prod_arr = Array();
	$tot_no_prod = $_REQUEST['totalProductCount'];
	$id = $_REQUEST['currentid'];
	$record_id = $_REQUEST['record'];
	...
	if($id){
		...
		{  	$prod_id = vtlib_purify($_REQUEST['hdnProductId'.$i]);
			...
			$select_to = vtlib_purify($_REQUEST['prd_mov_select_to'.$i]);
			$item_type = vtlib_purify($_REQUEST['lineItemType'.$i]);
			$rej_reason = vtlib_purify($_REQUEST['rej_reason'.$i]);
				if($i==1){ 
				//UPDATE `vtiger_movementrel` SET `id`=[value-1]
				$tem_arr="SELECT MAX(lineitem_id) AS max_id FROM vtiger_movementrel";
				$max_ids = $adb->pquery($tem_arr);
				...
				//UPDATE `vtiger_rej_reason` SET `id`=[value-1]
				$qry_reason="SELECT MAX(reason_id) AS maxm_id FROM vtiger_rej_reason";
				$maxm_res = $adb->pquery($qry_reason);
				$maxm_id = $adb->query_result($maxm_res, 0, 'maxm_id'); 
				$query_up = 'UPDATE vtiger_rej_reason SET lineitem_id =?, product_id=?, rej_reason=? 
								WHERE reason_id=?'; 
				$qparams_up = array($max_id,$prod_id,$rej_reason,$maxm_id); 
				$adb->pquery($query_up,$qparams_up);
				...
				$query_rzn = 'INSERT INTO vtiger_rej_reason(rejectionid, lineitem_id, product_id, rej_reason)
								VALUES(?,?,?,?)'; 
				$rzn_params = array($id,$max_id+1,$prod_id,$rej_reason);
				$adb->pquery($query_rzn,$rzn_params);
				}	
		$lineitem_id = $adb->getLastInsertID();
		$prod_seq++;
		}//exit('FORout');
	...
	if($record_id){
		...
		//For deletion of vtiger_rej_reason
		$line_select1 = 'DELETE FROM vtiger_rej_reason WHERE rejectionid=?';
		$rejectionid=array($record_id);
		$line_item_allIn = $adb->pquery($line_select1,$rejectionid);
		$prod_seq=1;
		for($i=1; $i<=$tot_no_prod; $i++) {  
			//echo $i.'##'.$tot_no_prod.'@@'.$record_id.'<br>';
			$prod_id = vtlib_purify($_REQUEST['hdnProductId'.$i]);
			$qty = vtlib_purify($_REQUEST['qty'.$i]);
			$select_frm = vtlib_purify($_REQUEST['prd_mov_select_frm'.$i]);
			$select_to = vtlib_purify($_REQUEST['prd_mov_select_to'.$i]);
			...
			//For in insertions vtiger_rej_reason
			//$qry_reason="SELECT MAX(reason_id) AS maxm_id FROM vtiger_rej_reason";
			//$maxm_res = $adb->pquery($qry_reason);
			//$maxm_id = $adb->query_result($maxm_res, 0, 'maxm_id'); 
			$tem_arr="SELECT MAX(lineitem_id) AS max_id FROM vtiger_movementrel";
			...
			$query_rzn = 'INSERT INTO vtiger_rej_reason(rejectionid, lineitem_id, product_id, rej_reason)
								VALUES(?,?,?,?)'; 
			$rzn_params = array($record_id,$max_id,$prod_id,$rej_reason);
			$adb->pquery($query_rzn,$rzn_params); 
		$prod_seq++;	
		}	//die('TFr');
	}
	$log->debug("Exit from function saveMovementProdRejectionDetails($module).");
}

function saveMovementBatchDetails(&$focus, $module)
{	//echo $focus.'##'.$module.'##'.$update_prod_stock.'##'.$updateDemand."<br>"; die('Hy');
	global $log, $adb;
	$id=$focus->id;
	$log->debug("Entering into function saveMovementBatchDetails($module).");
	$tot_no_prod = $_REQUEST['totalBatchCount'];
	$id = $_REQUEST['currentid'];
	$record_id = $_REQUEST['record'];
	if($id){
		$item_seq=1;
		for($i=1; $i<=$tot_no_prod; $i++)
		{  	$prod_id = vtlib_purify($_REQUEST['hdnBatchId'.$i]);
			$qty = vtlib_purify($_REQUEST['quantity'.$i]);
			$select_frm = vtlib_purify($_REQUEST['batcmov_select_frm'.$i]);
			$select_to = vtlib_purify($_REQUEST['batcmov_select_to'.$i]);
			$item_type = vtlib_purify($_REQUEST['lineItemType'.$i]);
			if($i==1){ 
			//UPDATE `vtiger_movementrel` SET `id`=[value-1]
			$tem_arr="SELECT MAX(lineitem_id) AS max_id FROM vtiger_movementrel";
			...
			$max_id = $adb->query_result($max_ids, 0, 'max_id');
			$query_up = 'UPDATE vtiger_movementrel SET itemid =?, sequence_no=?, quantity=?, select_frm=?, select_to=?, lineItem_type=? WHERE lineitem_id=?'; 
			$qparams_up = array($prod_id,$item_seq,$qty,$select_frm,$select_to,$item_type,$max_id); 
			$adb->pquery($query_up,$qparams_up); 
			} else { //INSERT vtiger_movementrel
			$query = 'INSERT INTO vtiger_movementrel(id, itemid, sequence_no, quantity, select_frm, select_to, lineItem_type)
						VALUES(?,?,?,?,?,?,?)';
			$qparams = array($id,$prod_id,$item_seq,$qty,$select_frm,$select_to,$item_type);
			$adb->pquery($query,$qparams);
			}	
		$lineitem_id = $adb->getLastInsertID();
		$item_seq++;
	...
	if($record_id){ 
		$line_select = 'DELETE FROM vtiger_movementrel WHERE id=?';
		$mov_id=array($record_id);
		//echo"<pre>";print_r($_REQUEST)."<br>"; die('4nd');	
		$line_item_all = $adb->pquery($line_select,$mov_id);
		$item_seq=1;
		for($i=1; $i<=$tot_no_prod; $i++)
		{  	$prod_id = vtlib_purify($_REQUEST['hdnBatchId'.$i]);
			...
			$query = 'INSERT INTO vtiger_movementrel(id, itemid, sequence_no, quantity, select_frm, select_to, lineItem_type)
						VALUES(?,?,?,?,?,?,?)';
			$qparams = array($record_id,$prod_id,$item_seq,$qty,$select_frm,$select_to,$item_type);
			$adb->pquery($query,$qparams);	
		$item_seq++;	
		}//exit('FOrLoop');	
	}
	$log->debug("Exit from function saveMovementBatchDetails($module).");
}

function saveInventoryProductSalesOrder(&$focus, $module)
{	global $log, $adb;
	//$id=$focus->id;
	$log->debug("Entering into function saveInventoryProductSalesOrder($module).");
	$tot_no_of_item = $_REQUEST['totalItemCount'];
	$id = $_REQUEST['currentid'];
	$record_id = $_REQUEST['record'];
	//echo"<pre>";print_r($_REQUEST); die;
	if($id){
		$item_seq=1;
		for($i=1; $i<=$tot_no_of_item; $i++)
		{  	$sales_id = vtlib_purify($_REQUEST['hdnSalesOrderId'.$i]);
			$prod_id = vtlib_purify($_REQUEST['hdnProductId'.$i]);
			
			if($i==1){ //echo "Hello"; 
			//UPDATE `vtiger_batch_item` SET `id`=[value-1]
			$tem_arr="SELECT MAX(batchItem_id) AS max_id FROM vtiger_batch_item"; 
			$max_ids = $adb->pquery($tem_arr);
			$max_id = $adb->query_result($max_ids, 0, 'max_id');
			$query_up = 'UPDATE vtiger_batch_item SET salesorderid=?, productid=?, sequence_no=? WHERE batchItem_id=?'; 
			$qparams_up = array($sales_id, $prod_id, $item_seq, $max_id); 
			$adb->pquery($query_up,$qparams_up); 
			} else { //INSERT vtiger_batch_item
			//echo "Biraj";
			$query = "INSERT INTO vtiger_batch_item(id, salesorderid, productid, sequence_no)
						VALUES(?,?,?,?)"; 
			$qparams = array($id,$sales_id,$prod_id,$item_seq); 
			$adb->pquery($query,$qparams);
			}	
		$lineitem_id = $adb->getLastInsertID();
		$item_seq++;
	...
	if($record_id){
		$line_select = 'DELETE FROM vtiger_batch_item WHERE id=?';
		$mov_id=array($record_id);
		$line_item_all = $adb->pquery($line_select,$mov_id);
		$item_seq=1;
		for($i=1; $i<=$tot_no_of_item; $i++)
		{  	$sales_id = vtlib_purify($_REQUEST['hdnSalesOrderId'.$i]);
			...
			$query = 'INSERT INTO vtiger_batch_item(id, salesorderid, productid, sequence_no)
						VALUES(?,?,?,?)';
			$qparams = array($record_id,$sales_id,$prod_id,$item_seq);
			$adb->pquery($query,$qparams);	
		$item_seq++;	
		}//exit('FOrLoop');	
	}
	$log->debug("Exit from function saveInventoryProductSalesOrder($module).");
}

function saveInventoryProductRelDispatch(&$focus, $module)
{	global $log, $adb;
	//$id=$focus->id;
	$log->debug("Entering into function saveInventoryProductRelDispatch($module).");
	$tot_no_of_item = $_REQUEST['totalItemCount'];
	$id = $_REQUEST['currentid'];
	$record_id = $_REQUEST['record'];
	//echo"<pre>";print_r($_REQUEST); die;
	if($id){
		...
		{  	
			$prod_id = vtlib_purify($_REQUEST['hdnProductId'.$i]);
			$quantity = vtlib_purify($_REQUEST['disp_quantity'.$i]);
			//$select_frm = vtlib_purify('Warehouse'.$i);
			$select_frm = vtlib_purify('Warehouse');
			//$select_to = vtlib_purify('Dispatch'.$i);
			$select_to = vtlib_purify('Dispatch');
			$item_type = vtlib_purify($_REQUEST['lineItemType'.$i]);
			if($i==1){ 
			//UPDATE `vtiger_batch_item` SET `id`=[value-1]
			$tem_arr="SELECT MAX(dispatchItem_id) AS max_id FROM vtiger_dispatch_item"; 
			$max_ids = $adb->pquery($tem_arr);
			$max_id = $adb->query_result($max_ids, 0, 'max_id');
			$query_up = 'UPDATE vtiger_dispatch_item SET productid=?, quantity=?, sequence_no=? WHERE dispatchItem_id=?'; 
			$qparams_up = array($prod_id, $quantity, $item_seq, $max_id); 
			$adb->pquery($query_up,$qparams_up);
			...
			$max_id = $adb->query_result($max_ids, 0, 'max_id');
			$query_up = 'UPDATE vtiger_movementrel SET itemid =?, sequence_no=?, quantity=?, select_frm=?, select_to=?, lineItem_type=? WHERE lineitem_id=?'; 
			$qparams_up = array($prod_id,$item_seq,$quantity,$select_frm,$select_to,$item_type,$max_id); 
			$adb->pquery($query_up,$qparams_up); 
			//echo"<pre>";print_r('Hy'); die;	
			} else { 
			//INSERT vtiger_batch_item
			$query = "INSERT INTO vtiger_dispatch_item(id, productid, quantity, sequence_no)
						VALUES(?,?,?,?)"; 
			$qparams = array($id,$prod_id,$quantity,$item_seq); 
			$adb->pquery($query,$qparams);
			//INSERT vtiger_movementrel
			$query = 'INSERT INTO vtiger_movementrel(id, itemid, sequence_no, quantity, select_frm, select_to, lineItem_type)
						VALUES(?,?,?,?,?,?,?)';
			$qparams = array($id,$prod_id,$item_seq,$quantity,$select_frm,$select_to,$item_type);
			$adb->pquery($query,$qparams);
			}	
		$lineitem_id = $adb->getLastInsertID();
		$item_seq++;
	...
	if($record_id){
		$line_select = 'DELETE FROM vtiger_dispatch_item WHERE id=?';
		$disp_id=array($record_id);
		$line_item_all = $adb->pquery($line_select,$disp_id);
		//Deletion for vtiger_movementrel table
		$line_select1 = 'DELETE FROM vtiger_movementrel WHERE id=?';
		$mov_id1=array($record_id);
		$line_item_all1 = $adb->pquery($line_select1,$mov_id1);
		//echo"<pre>";print_r($_REQUEST)."<br>"; die('4nd');	
		$item_seq=1;
		for($i=1; $i<=$tot_no_of_item; $i++)
		{  	$prod_id = vtlib_purify($_REQUEST['hdnProductId'.$i]);
			...
			$query = 'INSERT INTO vtiger_dispatch_item(id, productid, quantity, sequence_no)
						VALUES(?,?,?,?)';
			$qparams = array($record_id,$prod_id,$quantity,$item_seq);
			$adb->pquery($query,$qparams);	
			
			//INSERT vtiger_movementrel
			$query = 'INSERT INTO vtiger_movementrel(id, itemid, sequence_no, quantity, select_frm, select_to, lineItem_type)
						VALUES(?,?,?,?,?,?,?)';
			$qparams = array($record_id,$prod_id,$item_seq,$quantity,$select_frm,$select_to,$item_type);
			$adb->pquery($query,$qparams);
		$item_seq++;	
		}//exit('FOrLoop');	
	}
	$log->debug("Exit from function saveInventoryProductRelDispatch($module).");
}
