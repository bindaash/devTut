...
Class Dispatch_Edit_View extends Vtiger_Edit_View {
	...
	public function process(Vtiger_Request $request) {
		...
		if(!empty($record)  && $request->get('isDuplicate') == true) {
			...
			$recordModel = Dispatch_Record_Model::getInstanceById($record, $moduleName);
			$relatedProd = $recordModel->getProducts();
			//While Duplicating record, If the related record is deleted then we are removing related record info in record model
			$mandatoryFieldModels = $recordModel->getModule()->getMandatoryFieldModels();
			//print_r($mandatoryFieldModels); die('tr');
			...
			$recordModel = Dispatch_Record_Model::getInstanceById($record, $moduleName);
			$relatedProd = $recordModel->getProducts();
			//echo"<pre>";print_r($relatedProd); die('End');
			...
			if ($request->get('product_id') || $sourceModule === 'Products' || $request->get('productid')) {
				...
				$relatedProd = $productRecordModel->getDetailsForInventoryModule($recordModel);
			} 
		}
		...
		if ($request->get('displayMode') == 'overlay') { 
			...
			$viewer->view('EditView.tpl', 'Dispatch');
		}
	}
	...
	function getHeaderScripts(Vtiger_Request $request) { 
		...
		$jsFileNames = array(
				'modules.Dispatch.resources.Edit',
				'modules.Dispatch.resources.Popup',
		);
