...
class Dispatch_Detail_View extends Vtiger_Detail_View {
	...
	/**
	 * Function returns DispatchInventory details
	...
	function showModuleSummaryView($request) {
		...
		$relatedProd = $recordModel->getProducts();
		$finalDetails = $relatedProd[1];
		$relatedProd[1] = $finalDetails;
		$itemCount = count($relatedProd);
		for ($i=1; $i<=$itemCount; $i++) {
			$product = $relatedProd[$i];

			$relatedProd[$i] = $product;
		}
		$viewer = $this->getViewer($request);
		$viewer->assign('RELATED_ITEMS', $relatedProd);
	...
	/**
	 * Function returns DispatchInventory Line Items
	 * @param Vtiger_Request $request
	 */
	function showLineItemDetails(Vtiger_Request $request) {
		...
		//echo"<pre>";print_r($relatedProd);die;
		//$finalDetails = $relatedProd[1]['final_details'];
		$finalDetails = $relatedProd[1];
		$relatedProd[1] = $finalDetails;
		...
		$viewer->assign('RECORD', $recordModel);
		$viewer->assign('MODULE_NAME',$moduleName);
		//$viewer->view('LineItemsDetail.tpl', 'DispatchInventory');
	}
