...
class Batch extends Vtiger_CRMEntity {
        var $table_name = 'vtiger_batch';
        var $table_index= 'batchid';

        var $customFieldTable = Array('vtiger_batchcf', 'batchid');

        var $tab_name = Array('vtiger_crmentity', 'vtiger_batch', 'vtiger_batchcf', 'vtiger_batch_item');

        var $tab_name_index = Array(
                'vtiger_crmentity' => 'crmid',
                'vtiger_batch' => 'batchid',
                'vtiger_batchcf'=>'batchid',
                'vtiger_batch_item'=>'id');

        var $list_fields = Array (
                /* Format: Field Label => Array(tablename, columnname) */
                // tablename should not have prefix 'vtiger_'
                'Batch Name' => Array('batch', 'batch_name'),
                //'Sales Order Number' => Array('batch', 'so_number'),
                //'Product Name' => Array('batch', 'prd_id'),
				'Planning Date' => Array('batch', 'batch_date'),
                'Quantity' => Array('batch', 'batch_qty'),				                
        ...
        var $list_fields_name = Array (
               ...
               //'Product Name' => 'prd_id',
				'Planning Date' => 'batch_date',
                'Quantity' => 'batch_qty',                				
                'Assigned To' => 'assigned_user_id'
        );

        // Make the field link to detail view
        var $list_link_field = 'batch_name';
        ...
        var $popup_fields = Array ('batch_name');

        // For Alphabetical search
        var $def_basicsearch_col = 'batch_name';

        // Column value to use on detail view record text display
        var $def_detailview_recname = 'batch_name';

        // Used when enabling/disabling the mandatory fields for the module.
        // Refers to vtiger_field.fieldname values.
        var $mandatory_fields = Array('batch_name','assigned_user_id');

        var $default_order_by = 'batch_name';
        var $default_sort_order='ASC';
		
	function __construct(){
		global $log, $currentModule;
		$this->column_fields = getColumnFields('Batch');
		$this->db = new PearDatabase();
		$this->log = $log;
	}
	
	/** Constructor Function for Batch class
	*  This function creates an instance of LoggerManager class using getLogger method
	*  creates an instance for PearDatabase class and get values for column_fields array of Batch class.
	*/
	function Batch() { 
		$this->log =LoggerManager::getLogger('Batch');
		$this->db = PearDatabase::getInstance();
		$this->column_fields = getColumnFields('Batch');
	}
	function save_module($module){ 
		if(isset($_REQUEST)) {
			if($_REQUEST['action'] != 'BatchAjax' && $_REQUEST['ajxaction'] != 'DETAILVIEW'
					&& $_REQUEST['action'] != 'MassEditSave' && $_REQUEST['action'] != 'ProcessDuplicates'
					&& $_REQUEST['action'] != 'SaveAjax' && $_REQUEST['action'] != 'FROM_WS') {
				//Based on the total Number of rows we will save the vtiger_batch_item relationship with this entity
				//echo"<pre>"; print_r($_REQUEST); die('Ex');
				saveInventoryProductSalesOrder($this, 'Batch');
			} 
		}
	}	
	/**
	* Function to get Contact related Products
	* @param  integer   $id  - batchid
	...
	*/
	...
	/**	function used to get the list of sales orders which are related to the batch
	 *	@param int $id - batchid
