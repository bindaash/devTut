...
class Batch_ProductsPopup_View extends Vtiger_Popup_View {
	...
	public function initializeListViewContents(Vtiger_Request $request, Vtiger_Viewer $viewer) {
		...
		if(!$this->listViewEntries && $moduleModel->isActive()) {
			/*foreach($this->listViewEntries as $key_ent => $key_entry) {	
				$qty_per_unit = $key_entry->getField();
				echo"<pre>";print_r($qty_per_unit); die(0);
			}*/
			/*if($moduleModel->getField('qty_per_unit')){
				$qty_per_unit = $moduleModel->getField('qty_per_unit');
			//echo"<pre>";print_r($moduleModel->getField('qty_per_unit')); die(0);
			echo"<pre>";print_r($qty_per_unit); die(0);
			}*/
			$this->listViewEntries = $listViewModel->getListViewEntries($pagingModel);
			$salesOrderId = $request->get('saleOrderId');
			//print_r($salesOrderId);die;
			if($salesOrderId !='' && $salesOrderId != 0){
				$salesProdId = Batch_Module_Model::getRelatedProductBySalesOrder($salesOrderId);
				foreach($this->listViewEntries as $key => $listViewEntry) 
				{	$productId = $listViewEntry->getId();
					if(!in_array($productId, $salesProdId)){
						unset($this->listViewEntries[$productId]);
					}
				}
			}
		}else{
		...
		foreach($fieldList as $name => $model){
			...
			/*if($fieldsInfo[$name]['name'] == 'qty_per_unit'){
			echo"<pre>";print_r($fieldsInfo[$name]['name']); 
			}*/
		}
	...
	function getHeaderScripts(Vtiger_Request $request) {
		...
		$jsFileNames = array('modules.Batch.resources.Popup');
