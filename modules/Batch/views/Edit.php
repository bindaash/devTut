...
Class Batch_Edit_View extends Vtiger_Edit_View {
	...
	public function process(Vtiger_Request $request) {
		...
		if(!empty($record)  && $request->get('isDuplicate') == true) {
			...
			$recordModel = Batch_Record_Model::getInstanceById($record, $moduleName);
			//$relatedProdAndSales = $recordModel->getProducts();
			...
			$recordModel = Batch_Record_Model::getInstanceById($record, $moduleName);
			$relatedProdAndSales = $recordModel->getProductAndSalesOrder();
			//echo"<pre>";print_r($relatedProdAndSales); die('End');
			...
			if ($request->get('product_id') || $sourceModule === 'Products' || $request->get('productid')) {
				...
				$relatedProdAndSales = $productRecordModel->getDetailsForInventoryModule($recordModel);
			} 
		}
		...
		if ($request->get('displayMode') == 'overlay') { 
			...
			$viewer->view('EditView.tpl', 'Batch');
		}
	}
	...
	function getHeaderScripts(Vtiger_Request $request) { 
		...
		$jsFileNames = array(
				'modules.Batch.resources.Edit',
				'modules.Batch.resources.Popup',
		);
